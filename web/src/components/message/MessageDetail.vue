...
async function toggleStar() {
    if (!msg.value) return;
    await mailStore.starMessage(
        msg.value.id,
        msg.value.account_id,
        !msg.value.is_starred,
    );
}
...
<template>
    <div class="flex flex-col h-full">
        <!-- Loading -->
        <div v-if="loading" class="px-8 pt-8 space-y-4">
            ...
            <Skeleton class="h-64 w-full mt-4" />
        </div>

        <!-- Error -->
        <div
            v-else-if="error"
            class="flex items-center justify-center h-full text-destructive text-sm p-4 text-center"
        >
            {{ error }}
        </div>
        ...
        <template v-else>
            ...
            <div class="px-8 pt-8 pb-6 shrink-0">
                <div class="flex items-start justify-between gap-4">
                    <h1
                        class="text-xl font-semibold leading-snug tracking-tight break-words flex-1"
                    >
                ...
                <div
                    class="mt-5 grid grid-cols-[3.5rem_1fr] gap-x-3 gap-y-2 text-sm"
                >
            ...
            <Tabs default-value="rendered" class="flex flex-col flex-1 min-h-0">
                <TabsList
                    class="shrink-0 px-8 pt-3 justify-start rounded-none bg-transparent h-auto gap-2"
                >
                    <TabsTrigger value="rendered" class="text-xs h-8"
                        >Preview</TabsTrigger
                    >
                    <TabsTrigger value="text" class="text-xs h-8"
                        >Text</TabsTrigger
                    >
                    <TabsTrigger value="headers" class="text-xs h-8"
                        >Headers</TabsTrigger
                    >
                    <TabsTrigger
                ...
                <TabsContent value="rendered" class="flex-1 min-h-0 m-0 p-0">
                    ...
                    <ScrollArea v-else class="h-full">
                        <pre
                            class="px-8 py-6 text-sm leading-relaxed whitespace-pre-wrap font-mono text-foreground"
                            >{{ msg.body_text || "(empty)" }}</pre
                        >
                    </ScrollArea>
                </TabsContent>
                ...
                <TabsContent value="headers" class="flex-1 min-h-0 m-0">
                    <ScrollArea class="h-full">
                        <div class="px-8 py-6 text-xs font-mono">
                            <div
                    ...
                    <ScrollArea class="h-full">
                        <div class="px-8 py-6 space-y-3">
                            <a
                                ...
                                "
                                download
                                class="flex items-center gap-3 p-4 rounded-lg border border-border hover:bg-accent transition-colors text-sm"
                            >
                                <Download
